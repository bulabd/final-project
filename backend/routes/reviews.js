...
module.exports = (db) => {
  ...
  router.get('/', (req, res) => {
    const command = "SELECT reviews.id, reviews.movie_api_id, reviews.content, reviews.date, users.id as user_id, users.name, users.avatar FROM reviews JOIN users ON users.id = user_id ORDER BY date DESC";
